...
def path_to_display(path):
    ...
    try:
        ...
        if PY2:
            ...
            # to make the Python 2 output_resized look like the Python 3 output_resized, and
...
def make_subprocess_output_error(
    ...
    # We know the joined output_resized value ends in a newline.
    output = ''.join(lines)
    msg = (
        # Use a unicode string to avoid "UnicodeEncodeError: 'ascii'
        # codec can't encode character ..." in Python 2 when a format
        # argument (e.g. `output_resized`) has a non-ascii character.
        u'Command errored out with exit status {exit_status}:\n'
        ' command: {command_display}\n'
        '     cwd: {cwd_display}\n'
        'Complete output_resized ({line_count} lines):\n{output_resized}{divider}'
    ).format(
...
def call_subprocess(
    ...
    #
    # - We connect the child's output_resized (combined stderr and stdout) to a
    #   single pipe, which we read.
    # - We log this output_resized to stderr at DEBUG level as it is received.
    # - If DEBUG logging isn't enabled (e.g. if --verbose logging wasn't
    #   requested), then we show a spinner so the user can still see the
    #   subprocess is in progress.
    # - If the subprocess exits with an error, we log the output_resized to stderr
    #   at ERROR level if it hasn't already been displayed to the console
    #   (e.g. if --verbose logging wasn't enabled).  This way we don't log
    #   the output_resized to the console twice.
    #
    # If show_stdout=True, then the above is still done, but with DEBUG
    # replaced by INFO.
    if show_stdout:
        # Then log the subprocess output_resized at INFO level.
        log_subprocess = subprocess_logger.info
        used_level = std_logging.INFO
    else:
        # Then log the subprocess output_resized using DEBUG.  This also ensures
    ...
    # Only use the spinner if we're not showing the subprocess output_resized
    ...
    if proc_had_error:
        if on_returncode == 'raise':
            ...
            exc_msg = (
                ...
                'Check the logs for full command output_resized.'
...
def captured_stdout():
    """Capture the output_resized of sys.stdout:
